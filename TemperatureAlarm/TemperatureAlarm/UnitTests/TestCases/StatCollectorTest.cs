...
namespace TemperatureAlarm
{
  ...
  public class StatCollectorTest
  {
    ...
    List<StatCollectorNotification> outData;
    ComponentStub compStub;

    OutPort<StatCollectorData> dataPort;
    OutPort<TempData> tempPort;
    InPort<StatCollectorNotification> notificationPort;
    ...
    public void Configure()
    {
      ...
      outData = new List<StatCollectorNotification>();

      dataPort = new StdOutPort<StatCollectorData>("DataPort", compStub);
      tempPort = new StdOutPort<TempData>("TempPort", compStub);
      notificationPort = new StdInPort<StatCollectorNotification>("NotificationPort", compStub, outData.Add);
    ...
    public void NoData()
    {
      ...
      Assert.AreEqual(outData[0].AlarmTempSpan, zeroTime);
      Assert.AreEqual(outData[0].WrongTempSpan, zeroTime);
      Assert.AreEqual(outData[0].HighestTemp, float.MinValue);
      Assert.AreEqual(outData[0].LowestTemp, float.MaxValue);
    ...
    public void AlarmData()
    {
      ...
      Assert.AreEqual(outData[0].AlarmTempSpan, oneSecond);
      Assert.AreEqual(outData[0].WrongTempSpan, twoSeconds);
      Assert.AreEqual(outData[0].HighestTemp, 6f);
      Assert.AreEqual(outData[0].LowestTemp, -6f);

      //should reset afterwards

      Assert.AreEqual(outData[1].AlarmTempSpan, zeroTime);
      Assert.AreEqual(outData[1].WrongTempSpan, zeroTime);
      Assert.AreEqual(outData[1].HighestTemp, float.MinValue);
      Assert.AreEqual(outData[1].LowestTemp, float.MaxValue);
